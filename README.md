...
## The main idea
We Publish(an instance of Class) and We Subscribe(type of Class) to process. The type full name of Class will auto become an topic. Support distributed for subscrbiers with data structure queue, stack (one data push only one consumer process at a time) .topic (one data push can have many consumer process at a time)
